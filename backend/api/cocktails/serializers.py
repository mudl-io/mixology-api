...
class CocktailSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        liquor_ids = [liquor['id'] for liquor in Liquor.objects.filter(public_id__in=liquor_ids).values()]
        ingredient_ids = [ingredient['id'] for ingredient in Ingredient.objects.filter(public_id__in=ingredient_ids).values()]
